...
export function Chat({ onBrowserAction, onStreamingChange, onEvent, userPreferences }: ChatProps) {
  ...
  const handleEvent = (event: Event) => {
    switch (event.kind) {
      case "llm_response":
        // Parse Harmony channels from each iteration's LLM output
        if (event.content) {
          const parsed = parseHarmony(event.content);
          if (parsed.thinking) {
      ...
      case "final_answer": {
        ...
        setMessages((prev) => [...prev, {
          role: "assistant",
          content: parsed.final || event.content,
          thinking: currentThinking || parsed.thinking || undefined,
        }]);
        setCurrentThinking("");
        break;
      }
