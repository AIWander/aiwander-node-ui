...
export default function DashboardPage() {
  ...
  const [phasedRuns, setPhasedRuns] = useState<PhasedRun[]>([]);
  ...
  const fetchPanels = useCallback(async () => {
    try {
      const [statusRes, runsRes, flowsRes] = await Promise.allSettled([
        fetch("/api/state/status", { cache: "no-store" }),
        fetch("/api/runs/phased", { cache: "no-store" }),
        fetch("/api/workflow/flows", { cache: "no-store" }),
      ]);
      if (statusRes.status === "fulfilled" && statusRes.value.ok) {
        const s = await statusRes.value.json();
        setPlanContent(s.content || "");
        setPlanExists(s.exists || false);
      }
      if (runsRes.status === "fulfilled" && runsRes.value.ok) {
        const r = await runsRes.value.json();
        setPhasedRuns(r.runs || []);
      }
      if (flowsRes.status === "fulfilled" && flowsRes.value.ok) {
        const f = await flowsRes.value.json();
        setFlows(f.flows || []);
      }
    } catch { /* ignore */ }
  }, []);
  ...
  useEffect(() => {
    fetchData();
    fetchPanels();
    const id = setInterval(() => { fetchData(); fetchPanels(); }, 5000);
  ...
  return (
    ...
    >
      ...
      <div style={{ padding: "12px 16px", display: "flex", flexDirection: "column", gap: 12 }}>
        ...
        {/* ── Row 4: Plan of Action (full width) ─────────────────── */}
        <Card title="Plan of Action" icon={<FileText size={13} />}>
          {planExists ? (
            <pre
              style={{
                whiteSpace: "pre-wrap",
            ...
            <span style={{ color: C.textSec }}>
              No active plan. Run{" "}
              <code style={{ background: C.bg, padding: "2px 6px", borderRadius: 4, fontSize: 11 }}>
                bash /opt/cpc/scripts/run_phased.sh &lt;task&gt;
              </code>{" "}
              to generate one.
            </span>
          )}
        ...
        {/* ── Row 5: Phased Runs + Recorded Flows ─────────────────── */}
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
          {/* Phased Runs */}
          <Card title="Phased Runs" icon={<PlayCircle size={13} />}>
            {phasedRuns.length === 0 ? (
              <span style={{ color: C.textSec }}>No phased runs yet.</span>
            ) : (
              <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
                <div
                  style={{
                ...
                {phasedRuns.slice(0, 10).map((run, i) => (
                  <a
                    key={run.run_id}
                  ...
                  >
                    <span style={{ color: C.blue, fontVariantNumeric: "tabular-nums", fontSize: 11 }}>
                      {run.run_id}
                    </span>
                    <span
                      style={{ overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", color: C.textSec }}
                      title={run.task}
                    >
                      {run.task || "(no task)"}
                    </span>
                    <span style={{ fontVariantNumeric: "tabular-nums", color: C.textSec }}>
                      {run.plan_duration_ms >= 1000
                        ? `${(run.plan_duration_ms / 1000).toFixed(0)}s`
                        : `${run.plan_duration_ms}ms`}
                    </span>
                    <span style={{ fontVariantNumeric: "tabular-nums", color: C.textSec }}>
                      {run.exec_duration_ms >= 1000
                        ? `${(run.exec_duration_ms / 1000).toFixed(0)}s`
                        : `${run.exec_duration_ms}ms`}
                    </span>
                    <span style={{ fontVariantNumeric: "tabular-nums" }}>{run.total_tool_calls}</span>
                  </a>
                ))}
              </div>
            )}
          ...
          {/* Recorded Flows */}
          <Card title="Recorded Flows" icon={<Workflow size={13} />}>
            {flows.length === 0 ? (
              <span style={{ color: C.textSec }}>
                No recorded flows yet. Use{" "}
                <code style={{ background: C.bg, padding: "2px 6px", borderRadius: 4, fontSize: 11 }}>
                  flow_record_start
                </code>{" "}
                from a chat to begin.
              </span>
            ) : (
              <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                {flows.map((flow) => (
                  ...
                  >
                    <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                      <span style={{ fontWeight: 500, fontSize: 12 }}>{flow.name}</span>
                      <Badge text={`${flow.step_count} steps`} color={C.blue} />
                      <Badge text={flow.status} color={flow.status === "active" ? C.green : C.textSec} />
                    </div>
                    {flow.description && (
                      <div style={{ fontSize: 11, color: C.textSec, marginTop: 3 }}>{flow.description}</div>
                    )}
                    {flow.last_run && (
                      <div style={{ fontSize: 10, color: C.textSec, marginTop: 2 }}>
                        Last run: {flow.last_run}
                      </div>
                    )}
                  </div>
                ))}
